			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oIngEpay->setCachePath($this->aSettings['CACHE_PATH']);

						$aIssuerList = $oIngEpay->doIssuerRequest();
						$sIssuerList = '';
			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oIngEpay = new IngEpayPayments($this->aSettings['API_KEY']);
						$oIngEpay->setPaymentMethod('ideal');
						...
						$oIngEpay->setIssuer($sIssuerId);

						// NOT used?!
						// $oKassaCompleet->setLanguageCode($sLanguageCode);

						if($oIngEpay->getTransaction())
						{
							$this->oRecord['transaction_id'] = $oIngEpay->getTransactionId();
							$this->oRecord['transaction_url'] = $oIngEpay->getTransactionUrl();
